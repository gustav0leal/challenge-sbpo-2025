       ...
       // Build item-oriented maps
        ...
        try {
            ...
            for(int c = 0; c < BIN_ITER; c++)
            {
                ...
                if(getRemainingTime(stopWatch) <=50)
                    break;
                if(c == 4)
                {
                ...
                // }
                // double mid = l;
                // System.out.println("\n -------Execução " + c+" alpha "+alpha+" Tempo Disponivel "+(getRemainingTime(stopWatch)-100)/(BIN_ITER-c)+" Real time "+getRemainingTime(stopWatch)+" ------\n");
                ...
                cplex.setParam(IloCplex.DoubleParam.TiLim, (getRemainingTime(stopWatch)-50)/(BIN_ITER-c));
                if (cplex.solve()) {
        ...
        try {
            for(int outer=0; outer<BIN_ITER; outer++){
                ...
                for(int inner=0; inner<LAG_ITER; inner++){
                    ...
                    for(int i=0;i<nItems;i++) lambda[i]=Math.max(0, lambda[i]+tk*subg[i]);
